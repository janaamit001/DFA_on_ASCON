...
{0x11, 0x11, 0x01, 0x10, 0x00, 0x1}};
	...
	unsigned char tag_diff[5], rkey[64];
	u64 key_ret = 0;
	for(int j = 0; j < 64; ++j) {
	    for(int k = 0; k < 5; ++k) {
	    ...
	    for(short i = 0; i < 5; ++i)
	        printf("%02x ", tag_diff[i]);
	    printf("\n");
	    rkey[j] = search(&tag_diff[0], table);
	    printf("\n\n");
	}
	...
	unsigned char retrieved_key[16];
	U64_TO_BYTES(retrieved_key, key_ret, 16);
	
	for(short i = 0; i < 16; ++i) {
	
	    retrieved_key[i] ^= ct[32+i];
	}
	
	printf("\nThe last (or the least significant) 64-bit retrieved key (K_1)::\n");
	for(short i = 0; i < 16; ++i) {
