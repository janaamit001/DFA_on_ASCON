...
static inline void ROUND_fault_bs(u8 C, state* p, short c_pos, short r_pos) {
  state s = *p;
  state t;
  ...
  uint64_t z;
  z = ((s.x2 & (1ULL << 63-c_pos)) >> 63-c_pos);
  if(z == 0) {
      uint64_t y;
      y = 1ULL << 63-c_pos;
      if(r_pos == 0)
        s.x0 ^= y;//0x4000000000000000;
      if(r_pos == 1)
        s.x1 ^= y;
      if(r_pos == 2)
        s.x2 ^= y;
  ...
  //printf("size of u64 = %d, c_pos = %d, y=%016"PRIx64"\n\n", sizeof(uint64_t), c_pos, y);
...
static inline void ROUND_fault(u8 C, state* p, short c_pos, short r_pos) {
  state s = *p;
  state t;
  // addition of round constant
  s.x2 ^= C;
...
static inline void P12_fault_bs(state* s, short c_pos, short r_pos) {
  ...
  ROUND(0x69, s);
  ROUND(0x5a, s);
  ROUND_fault_bs(0x4b, s, c_pos, r_pos);
}


static inline void P12_fault(state* s, short c_pos, short r_pos) {
  printstate(" permutation input:", *s);
  ROUND(0xf0, s);
  ROUND(0xe1, s);
  ROUND(0xd2, s);
  ...
  ROUND(0x69, s);
  ROUND(0x5a, s);
  ROUND_fault(0x4b, s, c_pos, r_pos);
}
